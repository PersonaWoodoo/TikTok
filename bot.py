...
async def start_cmd(message: Message):
    ...
    text = (
        ...
        "/start_tiktok - запустить автоответчик\n"
        "/stop_tiktok - остановить автоответчик\n"
        "/status - статус работы\n"
        "/help - помощь\n"
    )
    
    if is_admin_user:
        text += "\n🔐 **Админ-команды:**\n/admin_stats - статистика всех аккаунтов\n/admin_account ID - детали аккаунта\n/admin_stop_all - остановить все сессии"
...
async def help_cmd(message: Message):
    ...
    text = (
        ...
        "/add - добавить аккаунт TikTok\n"
        "/my - список ваших аккаунтов\n"
        "/start_tiktok - запустить автоответчик TikTok\n"
        "/stop_tiktok - остановить автоответчик\n"
        "/status - статус работы\n\n"
    )
...
async def process_password(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            f"📱 Логин: `{username}`\n"
            f"🆔 ID: {account_id}\n\n"
            f"Теперь можно запустить автоответчик: /start_tiktok {account_id}",
            parse_mode="Markdown"
        )
    except Exception as e:
        logger.error(f"Ошибка: {e}")
        await message.answer("❌ Ошибка при сохранении аккаунта")
...
async def my_accounts_cmd(message: Message):
    ...
    for acc in accounts:
        ...
        created = acc['created_at'][:10] if acc['created_at'] else "неизвестно"
        text += f"{status} **ID {acc['id']}:** `{acc['username']}`\n"
        text += f"   └ {is_running} (добавлен: {created})\n\n"
    
    await message.answer(text, parse_mode="Markdown")

# ========== УПРАВЛЕНИЕ TIKTOK ВОРКЕРОМ ==========

@dp.message(Command("start_tiktok"))
async def start_tiktok_cmd(message: Message):
    ...
    text += "\nОтправьте: `/start_tiktok ID`\nПример: `/start_tiktok 1`"
...
async def _start_worker(message, account_id, username):
    ...
    try:
        ...
        await message.answer(
            ...
            f"🆔 ID: {account_id}\n\n"
            f"📌 Что делает:\n"
            f"• Раз в 7 часов отправляет случайный стикер/приветствие\n"
            f"• Отвечает на новые сообщения\n"
            f"• Поддерживает активность\n\n"
            f"Остановить: /stop_tiktok {account_id}",
            parse_mode="Markdown"
        )
    except Exception as e:
...
@dp.message(Command("stop_tiktok"))
async def stop_tiktok_cmd(message: Message):
    ...
    text += "\nОтправьте: `/stop_tiktok ID`\nПример: `/stop_tiktok 1`"
...
async def admin_stats_cmd(message: Message):
    ...
    text += f"⚪️ Неактивных: {total - active}\n\n"
    text += f"**Список всех аккаунтов:**\n\n"
    
    for acc in accounts:
        status = "🟢" if acc['active'] else "⚪️"
        running_status = "🚀" if acc['id'] in active_workers else "⏸"
        text += f"{status}{running_status} ID {acc['id']}: `{acc['username']}`\n"
        text += f"   └ Владелец: {acc['telegram_id']}\n"
...
async def admin_account_cmd(message: Message):
    ...
    args = message.text.split()
    if len(args) != 2:
        await message.answer("Использование: /admin_account ID")
        return
    ...
    text = (
        ...
        f"🟢 Статус в БД: {'Активен' if account['active'] else 'Неактивен'}\n"
        f"🚀 Работает: {'Да' if is_running else 'Нет'}\n"
        f"📅 Добавлен: {account['created_at']}\n"
    )
    
    if account['cookies']:
        text += f"\n🍪 Куки: `{account['cookies'][:100]}...`"
    
    await message.answer(text, parse_mode="Markdown")

@dp.message(Command("admin_stop_all"))
...
@dp.message(Command("admin_reload"))
async def admin_reload_cmd(message: Message):
    """Перезагрузка бота (админ)"""
    if not is_admin(message.from_user.id):
        await message.answer("⛔️ Доступ только для администратора!")
        return
    
    await message.answer("🔄 Перезагрузка бота...")
    logger.info("Перезагрузка по команде админа")
