...
async def handle_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "my_accounts":
        ...
        if not accounts:
            await callback.message.edit_text("📭 У вас нет аккаунтов.\n\nНажмите ➕ Добавить аккаунт", reply_markup=get_back_keyboard())
        else:
            text = "📱 Ваши аккаунты:\n\n"
            for acc in accounts:
    ...
    if data.startswith("start_"):
        ...
        if account and account_id not in active_workers and account['active']:
            ...
            await callback.message.edit_text(f"✅ Автоответчик запущен для {account['username']}", reply_markup=get_back_keyboard())
            await callback.answer("✅ Запущено!", show_alert=True)
        else:
            await callback.answer("❌ Ошибка или аккаунт не авторизован", show_alert=True)
        return
...
async def add_password(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "⏳ Подождите, это может занять 15-30 секунд",
        reply_markup=get_back_keyboard()
    )
    
    # Запускаем процесс входа
    success, need_code, cookies = await _perform_tiktok_login(username, password, message.from_user.id)
    
    if success and cookies:
        # Сохраняем куки и активируем аккаунт
...
async def process_2fa_code(message: Message, state: FSMContext):
    ...
    # Здесь должен быть код для подтверждения 2FA
    ...
    await message.answer(
        ...
        f"🆔 ID: {account_id}\n\n"
        f"ВНИМАНИЕ: Для входа в TikTok может потребоваться ручное подтверждение.\n"
        f"Проверьте аккаунт и при необходимости войдите вручную один раз.",
        reply_markup=get_main_keyboard(is_admin(message.from_user.id))
    )
...
# ========== ФУНКЦИЯ ВХОДА В TIKTOK ==========

async def _perform_tiktok_login(username, password, telegram_id):
    """Выполняет вход в TikTok и возвращает результат"""
    try:
        async with async_playwright() as p:
            ...
            logger.info(f"🌐 Переход на TikTok для {username}")
            ...
            # Проверяем, появилось ли поле для 2FA кода
            code_input = await page.query_selector("input[inputmode='numeric']")
            
            if code_input:
                logger.info(f"📧 Требуется 2FA код для {username}")
                await browser.close()
            ...
            if "login" not in current_url:
                ...
                return True, False, cookies
            else:
                logger.error(f"❌ Вход не удался для {username}")
                await browser.close()
