...
async def init_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        # Таблица для хранения аккаунтов TikTok
        await db.execute('''
        ...
        await db.execute('CREATE INDEX IF NOT EXISTS idx_telegram_id ON accounts(telegram_id)')
        await db.execute('CREATE INDEX IF NOT EXISTS idx_username ON accounts(username)')
...
async def add_account(telegram_id, username, password):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "INSERT INTO accounts (telegram_id, username, password, active) VALUES (?, ?, ?, 0)",
...
async def get_all_accounts():
    """Получить все аккаунты из базы (для админа)"""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
...
async def get_account_by_id(account_id):
    """Получить полную информацию об аккаунте по ID"""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
...
async def update_account_status(account_id, active):
    """Обновить статус аккаунта (активен/неактивен)"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE accounts SET active = ?, last_active = CURRENT_TIMESTAMP WHERE id = ?",
            (1 if active else 0, account_id)
        )
        await db.commit()

async def update_account_cookies(account_id, cookies):
    """Обновить cookies аккаунта после успешного входа"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def get_active_accounts_count():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT COUNT(*) FROM accounts WHERE active = 1") as cursor:
            count = await cursor.fetchone()
            return count[0] if count else 0
...
async def get_users_count():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT COUNT(DISTINCT telegram_id) FROM accounts") as cursor:
            count = await cursor.fetchone()
            return count[0] if count else 0
